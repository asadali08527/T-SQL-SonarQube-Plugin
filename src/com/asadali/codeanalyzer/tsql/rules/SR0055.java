...
public class SR0055 extends AbstractSyntaxTreeBasedRule {
	...
	private void searchReductionTree(Reduction reduction) {
		for (Token token : reduction) {
			switch (token.getType()) {
				case NON_TERMINAL:				
					if ("<DeleteStatement>".equals(token.asReduction().getParent().getHead().toString())) {
	...
	public String description() {		
		return "Delete statement should have a \"Where\" clause";
	}
